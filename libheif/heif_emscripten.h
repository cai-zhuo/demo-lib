...
static emscripten::val heif_js_get_metadata(struct heif_image_handle *handle)
{
  emscripten::val result = emscripten::val::array();
  ...
  for (int i = 0; i < count; i++)
  {
    ...
    uint8_t *metadata = static_cast<uint8_t *>(malloc(datasize));
    ...
    heif_error error = heif_image_handle_get_metadata(handle, metadata_id, metadata);
...
EMSCRIPTEN_BINDINGS(libheif)
{
  ...
  emscripten::function("heif_js_get_metadata",
                       &heif_js_get_metadata, emscripten::allow_raw_pointers());
